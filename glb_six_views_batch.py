...
import argparse
import time
from glb_six_views_main import process_model
...
def process_directory(directory_path, output_dir=None, resolution=1000, keep_temp=False):
    ...
    if not glb_files:
        print(f"在目录 {directory_path} 中未找到GLB文件")
        return
    
    print(f"找到 {len(glb_files)} 个GLB文件待处理")
    ...
    for i, glb_file in enumerate(glb_files, 1):
        print(f"\n[{i}/{len(glb_files)}] 处理文件: {os.path.basename(glb_file)}")
        
        # 设置输出路径
        if output_dir:
            model_name = os.path.splitext(os.path.basename(glb_file))[0]
            output_path = os.path.join(output_dir, f"{model_name}_six_views.jpg")
        else:
            output_path = None
        ...
        try:
            ...
            if result:
                print(f"成功处理: {os.path.basename(glb_file)} -> {result}")
                successful += 1
            else:
                print(f"处理失败: {os.path.basename(glb_file)}")
                failed += 1
        except Exception as e:
            print(f"处理 {os.path.basename(glb_file)} 时出错: {e}")
            failed += 1
    
    # 打印处理结果统计
    print(f"\n批处理完成: 总共 {len(glb_files)} 个文件, 成功 {successful} 个, 失败 {failed} 个")

def main():
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='批量处理GLB模型：渲染六个标准视图并合并为一个图像')
    parser.add_argument('directory_path', help='包含GLB文件的目录路径')
