...
def render_with_blender(model_path, output_dir=None, resolution=1000):
    ...
    blender_script = os.path.join(script_dir, "blender_six_views.py")
...
def combine_views(view_paths, output_path, model_name):
    """
    将六个视图图像合并为一个图像，并添加标签
    
    Args:
        view_paths: 包含六个视图路径的字典
        output_path: 输出图像路径
    ...
    # 检查是否有所有需要的视图
    required_views = ['正面', '左视图', '背面', '右视图', '俯视图', '底视图']
    ...
    positions = {
        ...
        '右视图': (padding, title_height + padding * 2 + img_height + label_height),
        '俯视图': (padding * 2 + img_width, title_height + padding * 2 + img_height + label_height),
        '底视图': (padding * 3 + img_width * 2, title_height + padding * 2 + img_height + label_height)
    }
...
def process_model(model_path, output_path=None, resolution=1000, keep_temp=False):
    ...
    Args:
        model_path: 模型文件路径
        output_path: 输出图像路径，如果为None则使用模型名称_six_views.jpg
        resolution: 渲染分辨率
        keep_temp: 是否保留临时文件
    """
    ...
    if output_path is None:
        ...
        model_name = os.path.splitext(os.path.basename(model_path))[0]
        output_path = os.path.join(model_dir, f"{model_name}_six_views.jpg")
    ...
    # 检查所有视图是否存在
    missing_views = []
    for view_type in ['正面', '左视图', '背面', '右视图', '俯视图', '底视图']:
...
def main():
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='渲染GLB模型的六个标准视图并合并为一个图像')
